    ...
    if (postRegex.test(to.fullPath) || postRegex.test(from.fullPath)) {
      return {
        ...
        async enter(el, done) {
          ...
          const containers = el.querySelectorAll(
            `.post-container:not([data-id="${postId}"])`
          );

          const containerPosition = elementPosition(container);
          ...
          await moveToCoords(clone, containerPosition);

          fadeIn(containersInView);
      ...
      return {
        ...
        enter(el, done) {
          ...
          console.log(containersInView, window.scrollY);
